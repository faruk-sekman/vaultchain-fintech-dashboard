 ...
 *
 * Parses + validates the customer KYC-verification-list paging query (api-endpoint-specifications
 * §GET /customers/{id}/kyc-verifications). Bracketed flat keys (Fastify default parser); only
...
export interface ParsedKycListQuery {
  page: number;
  size: number;
}
...
export function parseKycListQuery(raw: Record<string, unknown>): ParsedKycListQuery {
