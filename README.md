...
| **Customers & KYC** | Searchable, filterable, paginated customer roster (non-sensitive status/page state synced to the URL; free-text search stays local so PII never enters browser history); a defined KYC state machine with an append-only verification history; a Customer 360° screen combining identity + wallet + transactions + risk history. |
